...
export default function PreviewScreen() {
  ...
  const listType = getStringParam(params?.listType, 'Normal List');
  const parsedItemsValue = safeJsonParseParam(params?.items, [], 'preview items');
  const parsedItems = Array.isArray(parsedItemsValue) ? parsedItemsValue : [];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Save button */}
      <TouchableOpacity
        style={[styles.saveBtn, { bottom: insets.bottom + 16 }, isSaving && styles.saveBtnDisabled]}
        onPress={handleSave}
