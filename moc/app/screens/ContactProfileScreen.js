...
export default function ContactProfileScreen() {
  ...
  const name = getStringParam(params?.name, 'Contact');
  const image = getStringParam(params?.image, '');
  const avatarUri = getAvatarUri(image);
  const phone = getStringParam(params?.phone, '');
  const sharedMediaValue = safeJsonParseParam(params?.media, [], 'shared media');
  const sharedMedia = Array.isArray(sharedMediaValue) ? sharedMediaValue : [];
